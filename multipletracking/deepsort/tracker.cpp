...
void tracker::update(const DETECTIONS & detections)
{
  ...
  for (Track & track:tracks) {
        ...
        FEATURESS t = FEATURESS(0, Feature_Vector_Dim);
        track.features = t;
    }
...
void tracker::update(const DETECTIONSV2 & detectionsv2)
{
    ...
    TRACHER_MATCHD res;
    _match(detections, res);
    std::cout << "[deepsort] Match detection with last frame done!" << std::endl;
    ...
    std::cout << "[deepsort] KalmanFilter update object done!" << std::endl;
...
void tracker::_match(const DETECTIONS & detections, TRACHER_MATCHD & res)
{
    ...
    std::cout << "[deepsort] Track confirme&unconfirm done!" << std::endl;
    ...
    TRACHER_MATCHD matcha = linear_assignment::getInstance()-> matching_cascade(
        ...
        detections,
        confirmed_tracks);
    std::cout << "[deepsort] Track linear assignment matching cascade done!" << std::endl;
    ...
    std::cout << "[deepsort] Track linear assignment done!" << std::endl;
    //unmatched_tracks;
