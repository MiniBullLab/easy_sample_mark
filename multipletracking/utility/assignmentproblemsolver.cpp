...
double AssignmentProblemSolver::Solve(const std::vector<std::vector<double> >& DistMatrix, std::vector<int>& Assignment)
{
...
void AssignmentProblemSolver::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
    ...
    *cost = 0;
    ...
    for (row = 0; row<nOfElements; row++)
    {
        ...
        if (value < 0)
        {
            std::cout << "All matrix elements have to be non-negative." << std::endl;
        }
